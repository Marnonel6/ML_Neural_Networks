...
class Perceptron():
    ...
    def update_weights(self, X, y):
        ...
        # print(f"\n self.Weights.T.shape = {self.weights.T}")
        
        # print(f"\n (self.weights.T*X)*y Test = {np.matmul(self.weights.T,X[0])*y[0]}")

        converge = 1

        # print(f"\n (self.weights.T*X)*y = {np.matmul((np.matmul(np.transpose(self.weights),X[0])),y)}")

        for i in range(0,X.shape[0]):
            if np.matmul(self.weights.T,X[i])*y[i] > 0:
                print(f"\n Match {i}")
            else:
                self.weights = self.weights + X[i]*y[i]
                # print(f"\n ------------ Fail {i}")
                converge = 0
    ...
    def predict(self, X):
        ...
        for i in range(0,X.shape[0]):
            g = np.matmul(self.weights.T,X[i])
            if g > 0:
                predictions[i] = 1
            else:
                predictions[i] = -1
