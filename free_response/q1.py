...
def load_digits(classes=range(10), n=None):
    ...
    Helper function to load full MNIST dataset
        ...
        of MNIST data to `data/openml/openml.org/data/v1/download/62667.gz`
...
def two_class():
    ...
    """
    # Consider changing these lines for your answers in Q1
    X, y = load_digits(classes=(6,9), n=None)
...
def ten_class():
    ...
    On Zach's M1 Macbook, this takes ~90 seconds to run
    """
    ...
    X, y = load_digits(classes=range(10), n=None)
    np.random.seed(42)
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=0.5, random_state=42)
